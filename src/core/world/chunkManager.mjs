...
import { GravityQueue } from "../systems/gravity.mjs";
 ...
 * @typedef {import('./config/blocks.mjs').BlockMetadata} BlockMetadata
...
/**
 * @typedef {Object} StoredPlantState
 *
...
export class ChunkManager {
  ...
  constructor() {
    ...
    /** @type {Blocks|null} Block definitions */
    ...
    /** @type {Map<string, StoredPlantState>} Stored plant states for unloaded chunks */
    ...
    for (let i = 0; i < workerCount; i++) {
      const worker = new Worker(
        new URL("./generation/terrain.worker.mjs", import.meta.url),
        { type: "module" },
      );
  ...
  handleWorkerMessage(e) {
    ...
    if (chunk) {
      ...
      if (this.blockTypes) {
        propagateLight(chunk, this.blockTypes, this);
      }
   ...
   *
   * @param {Chunk} chunk
   * @param {Object<string, any>|null} [growthTimers=null]
   * @param {Object<string, any>|null} [plantStructures=null]
   * @param {function|null} [onPlantsRestored=null]
   */
  restoreChunkPersistence(
    ...
    if (this.storedPlantStates.has(key)) {
      ...
      const plantState = this.storedPlantStates.get(key);
      if (plantState) {
        const { timers, structures } = plantState;
        ...
        if (plantStructures) {
          for (const [structureKey, structure] of Object.entries(structures)) {
            /** @type {any} */ (plantStructures)[structureKey] = structure;
          }
        }
        ...
        if (onPlantsRestored && plantStructures) {
          const validStructures = Object.keys(structures).filter(
            (key) => /** @type {any} */ (plantStructures || {})[key],
          );
   ...
   * @param {BlockMetadata|null} [metadata=null] - Optional metadata for the block
  ...
  setBlock(
    ...
    if (result) {
      ...
      if (this.blockTypes && !skipLighting) {
        updateLightOnBlockChange(this, floorX, floorY, floorZ, this.blockTypes);
      }
    }
   ...
   * @param {boolean} [options.skipLighting=false]
   */
   ...
   * @param {number} localZ - Local z within chunk
   */
   ...
   * @param {number} y - World y coordinate
   * @param {number} z - World z coordinate
   * @param {boolean} [isPlayerChange=false] - Whether this is a player action
   *
   * @returns {boolean}
   */
   ...
   * @param {number} playerZ - Player world z coordinate
   * @param {number} seed - World generation seed
   * @param {WebGLRenderingContext} gl - WebGL context
   * @param {Function} deleteChunkMesh - Function to delete chunk mesh
   * @param {Object|null} [growthTimers=null] - Plant growth timer data
   * @param {Object|null} [plantStructures=null] - Plant structure data
   * @param {Function|null} [onPlantsRestored=null] - Callback when plants restored
   */
  updateVisibleChunks(
    playerX,
    playerZ,
      ...
      if (dx > cachedRadius || dz > cachedRadius) {
        ...
        if (growthTimers && plantStructures) {
          ...
          for (const timerKey of Object.keys(growthTimers)) {
            ...
            const { chunkX: cX, chunkZ: cZ } = worldToChunk(x, z);

            if (cX === chunk.chunkX && cZ === chunk.chunkZ) {
              /** @type {any} */
              const gt = growthTimers;
              chunkTimers[timerKey] = gt[timerKey];
          ...
          for (const structKey of Object.keys(plantStructures)) {
            ...
            if (cX === chunk.chunkX && cZ === chunk.chunkZ) {
              /** @type {any} */
              const ps = plantStructures;
              chunkStructures[structKey] = ps[structKey];
   ...
   *
   * @param {Function} callback - Function to call for each block key and type
   */
  forEach(callback) {
    for (const chunk of this.chunks.values()) {
  ...
  *entries() {
   ...
   *
   * @param {string} key - Block key in format "x,y,z"
   *
   * @returns {boolean}
   */
  has(key) {
   ...
   * @param {boolean} [isPlayerChange=false] - Whether this is a player action
   * @param {Object|null} [metadata=null] - Block metadata
   * @param {boolean} [skipLighting=false] - Whether to skip lighting updates
   *
