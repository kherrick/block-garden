...
export const COLOR_STORAGE_KEY = "block-garden-custom-colors";
...
export class ColorCustomizationDialog {
  ...
  async handleSave() {
    ...
    this.shadow.dispatchEvent(
      new CustomEvent("block-garden-reset", {
        detail: {
          colors: this.colors,
        },
      }),
  ...
  renderColorInputs() {
    ...
    for (const [property, value] of Object.entries(this.colors)) {
      // Extract category from property name (e.g., --bg-color-air, --bg-color-gray-50)
      const match = property.match(/--bg-?([a-z]+)-/);
    ...
    for (const category of sortedCategories) {
      ...
      for (const { property, value } of grouped[category]) {
        ...
        if (property.startsWith("--bg-color")) {
